...
import { IGeomStyle } from "@/types/typeDeclarations.ts";
import _ from "@/utils/lodash";
import { renderToStaticMarkup } from "react-dom/server";

const minPointSize = 9,
  maxPointSize = 25;
...
export const getPointSVG = (
  ...
  const pointSize = style?.pointSize || maxPointSize;
  const ratio = (maxPointSize - minPointSize) / (pointSize - minPointSize);
  ...
  switch (style.pointShape) {
    ...
    case "triangle":
      ...
      break;
    default:
      svgShape = <circle cx={w} cy={h} r={size / 2} style={svgStyle}></circle>;
