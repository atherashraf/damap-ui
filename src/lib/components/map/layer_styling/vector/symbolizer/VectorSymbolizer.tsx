...
class VectorSymbolizer extends React.PureComponent<IProp, IState> {
    ...
    componentDidUpdate(prevProps: Readonly<IProp>) {
        if (!_.isEqual(prevProps.style, this.props.style)) {
            ...
            this.setState({
                //@ts-ignore
                strokeWidth: style?.strokeWidth, //@ts-ignore
                strokeColor: style?.strokeColor, //@ts-ignore
    ...
    getStyleParams(): IGeomStyle | undefined {
        ...
        if (!isError) {
            ...
            // ⬇️ Pull point settings from PointSymbolizer
            const ps = this.pointSymbolRef?.current?.getPointSymbolizer?.();
            if (ps) {
                params.pointSize = ps.pointSize;
                params.pointShape = ps.pointShape;

                // ⬇️ Pass the uploaded (auto-resized) icon up if present
                if (ps.pointIconSrc) {
                    params.pointIconSrc = ps.pointIconSrc; // NEW
                }
            }

            // ⬇️ your existing stroke/fill plumbing
                ...
                <Box sx={{flex: 1}}>
                    <fieldset>
                        ...
                        <TextField
                            ...
                            InputProps={{
                                inputProps: {
                                    min: this.minStrokeWidth, max: this.maxStrokeWidth,
                                },
                            }}
                        />
                    </fieldset>
